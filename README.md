...
大模型配置统一采用 `LLM_*` 前缀，当前通过 `LLM_API_KEY`、`LLM_BASE_URL`、`LLM_TIMEOUT_SECONDS`、`LLM_CHAT_MODEL`、`LLM_IMAGE_RECOGNITION_MODEL`、`LLM_IMAGE_RERANK_MODEL` 配置；旧变量 `DASHSCOPE_API_KEY`、`DASHSCOPE_BASE_URL`、`BAILIAN_TIMEOUT_SECONDS`、`BAILIAN_CHAT_MODEL`、`IMAGE_RECOGNITION_MODEL`、`LLM_RERANK_MODEL`、`RERANK_MODEL` 仍兼容。
