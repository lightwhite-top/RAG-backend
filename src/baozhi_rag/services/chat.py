...
class ChatService:
    ...
    def _prepare_completion(
        ...
        retrieval_query = self._resolve_retrieval_query(normalized_messages)
        hits = self._chunk_search_service.search(retrieval_query, retrieval_size)
        citations = [self._build_citation(hit) for hit in hits]
    ...
    def _build_citation(self, hit: ChunkSearchHit) -> ChatCitation:
        """把检索命中结果转换为聊天引用对象。"""
        return ChatCitation(
